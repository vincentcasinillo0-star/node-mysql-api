...
export function fakeBackendInterceptor(request: HttpRequest<any>, next: HttpHandlerFn): Observable<any> {
  ...
  function getAccountById() {
    ...
    const currentAccount = accounts.find(x => x.id === currentAccountId());
    if (account.id !== currentAccountId() && currentAccount?.role !== Role.Admin) {
      return unauthorized();
    }
  ...
  function deleteAccount() {
    ...
    // user accounts can delete own account and admin accounts can delete all accounts
    const currentAccount = accounts.find(x => x.id === currentAccountId());
    if (account.id !== currentAccountId() && currentAccount?.role !== Role.Admin) {
      return unauthorized();
    }
  ...
  function getRefreshToken() {
    ...
    return document.cookie.split(';').map(c => c.trim()).find(c => c.startsWith('fakeRefreshToken='))
      ?.split('=')[1];
  }
